...
document.addEventListener('DOMContentLoaded', () => {
    ...
    toggleSwitch.addEventListener('change', () => {
        if (toggleSwitch.checked) {
            // Toggle is ON - clear notifications with enhanced "fly away" effect
            isActive = false;
            ...
            // Центр экрана (точка, от которой разлетаются уведомления)
            const centerX = window.innerWidth / 2;
            const centerY = window.innerHeight / 2;
            
            // Размеры экрана для расчета расстояния полета
            const screenWidth = window.innerWidth;
            const screenHeight = window.innerHeight;
            const screenDiagonal = Math.sqrt(screenWidth * screenWidth + screenHeight * screenHeight);
            
            // Анимируем разлет каждого уведомления от центра экрана
            notifications.forEach((notification) => {
                // Получаем текущую позицию уведомления
                const rect = notification.getBoundingClientRect();
                const notifX = rect.left + rect.width / 2;
                const notifY = rect.top + rect.height / 2;
                
                // Рассчитываем вектор от центра экрана к уведомлению
                const vectorX = notifX - centerX;
                const vectorY = notifY - centerY;
                ...
                // Рассчитываем дистанцию полета (1.5 диагонали экрана)
                const flyDistance = screenDiagonal * 1.5;
                
                // Целевые координаты полета в том же направлении, но за пределы экрана
                const targetX = normalizedX * flyDistance;
                const targetY = normalizedY * flyDistance;
                
                // Добавляем случайное вращение при полете
                const randomRotation = (Math.random() * 360) - 180; // От -180 до 180 градусов
                
                // Создаем частицы в позиции уведомления перед анимацией
                createAppearanceParticles(notifX, notifY);
            ...
            // Очищаем массив уведомлений после разлета
            setTimeout(() => {
                notifications = [];
            }, 1000);
        } else {
